...
interface DateSpecificPettyCashDialogProps {
  open: boolean;
  onClose: () => void;
  date: string;
}
...
const DateSpecificPettyCashDialog: FC<DateSpecificPettyCashDialogProps> = ({
  open,
  onClose,
  date,
}) => {
  ...
  // Filter orders with transactions for the specific date
  const specificDate = dayjs(date);
  const paidPeriodOrders =
    rentalOrders?.filter(
      (order) =>
        (order.balance_paid_date && dayjs(order.balance_paid_date).isSame(specificDate, 'day')) ||
        (order.repay_date && dayjs(order.repay_date).isSame(specificDate, 'day')) ||
        order.deposits.some((deposit) => dayjs(deposit.date).isSame(specificDate, 'day'))
    ) || [];
  ...
  const processedRowData = paidPeriodOrders
    .map((order) => {
      ...
      // Determine the transaction date that matched the specific date
      let transactionDate = null;
      if (order.balance_paid_date && dayjs(order.balance_paid_date).isSame(specificDate, 'day')) {
        transactionDate = order.balance_paid_date;
      } else if (order.repay_date && dayjs(order.repay_date).isSame(specificDate, 'day')) {
        transactionDate = order.repay_date;
      } else {
        const matchingDeposit = order.deposits.find((deposit) =>
          dayjs(deposit.date).isSame(specificDate, 'day')
        );
        transactionDate = matchingDeposit ? matchingDeposit.date : order.invoice_date;
      }

      // Calculate deposits made on the specific date by mode
  ...
  const periodPettyCashes =
    pettyCashes?.filter((pettyCash) => dayjs(pettyCash.created_date).isSame(specificDate, 'day')) ||
    [];
...
export default DateSpecificPettyCashDialog;
